...
export default function InstallerSelector() {
  ...
  const isCommandValid = !(
    skillMode === "specific" && selectedSkills.length === 0
  );
      ...
      {activeTab === "cli" ? (
        <div className="flex flex-col gap-6 animate-in fade-in duration-200">
          <p className="text-sm text-text-secondary -mb-2">
            ...
            (which includes <code>npx</code>). If you do not have Node.js
            installed, please switch to the <strong>Manual Installation</strong>{" "}
            tab above.
          </p>
          {/* Controls Box */}
          <div className="flex flex-col gap-6 bg-bg-secondary border border-border-primary p-6 rounded-xl shadow-sm">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4">
              <label
                ...
                className="text-sm font-semibold text-text-secondary md:w-[170px]"
              >
                Target AI Agent:
              </label>
              <select
                ...
                {skillMode === "specific" && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 bg-bg-primary border border-border-primary p-4 rounded-lg animate-in fade-in slide-in-from-top-2 duration-200">
                    {ALL_SKILLS.map((skill) => (
                      <label
                        key={skill.id}
                        className="flex items-center gap-2.5 text-xs text-text-secondary hover:text-text-primary cursor-pointer transition-colors font-medium select-none"
                      >
                        <input
                          type="checkbox"
                ...
                <div className="flex items-center justify-between p-3.5 bg-bg-primary rounded-lg border border-border-primary shadow-2xs">
                  <div className="flex flex-col pr-3">
                    ...
                    <span className="text-[11px] text-text-muted">
                      Install in local project folder
                    </span>
                  </div>
                  <button
                    type="button"
            ...
            <div className="bg-[#1e293b] px-4 py-3 flex justify-between items-center border-b border-[#334155]">
              ...
              >
                {copied ? (
                  ...
                  <>
                    <Copy size={16} />
                    <span className="text-xs font-semibold text-slate-400 hover:text-slate-100">
                      Copy Command
                    </span>
                  </>
                )}
        ...
        <div className="flex flex-col gap-6 animate-in fade-in duration-200">
          ...
          <div className="flex flex-col gap-4 bg-bg-secondary border border-border-primary p-6 rounded-xl shadow-sm">
            ...
            <p className="text-sm text-text-secondary leading-relaxed">
              Download the compiled <code>skills.zip</code> package containing
              all core Spec-First Protocol agent skills.
            </p>
            ...
            <p className="text-xs text-text-muted mt-2 border-t border-border-primary pt-3">
              💡 <strong>Network issues?</strong> If direct downloads fail,
              browse and select release archives directly from the backup{" "}
              <a
                href="https://github.com/awhipp/spec-first-protocol/releases"
                target="_blank"
                rel="noopener noreferrer"
          ...
          <div className="flex flex-col gap-4 bg-bg-secondary border border-border-primary p-6 rounded-xl shadow-sm">
            ...
            <ol className="list-decimal list-inside text-sm text-text-secondary space-y-3.5 pl-1 leading-relaxed">
              <li>
                Extract the contents of the downloaded <code>skills.zip</code>{" "}
                archive.
              </li>
              <li>
                Copy the extracted directories (<code>sfp-discover</code>,{" "}
                <code>sfp-audit</code>, <code>sfp-refine</code>,{" "}
                <code>sfp-orchestrate</code>, <code>sfp-personas</code>)
                directly into your target agent config directory.
              </li>
            </ol>
          </div>
          ...
          <div className="flex flex-col gap-4 bg-bg-secondary border border-border-primary p-6 rounded-xl shadow-sm">
            ...
            <p className="text-sm text-text-secondary leading-relaxed">
              Common destination directories for manual setup on typical
              environments:
            </p>
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border-primary">
                    <th className="py-2.5 px-3 font-semibold text-text-primary w-1/3">
                      AI Agent / Editor
                    </th>
