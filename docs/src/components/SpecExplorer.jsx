...
import { FileText, ChevronDown, ChevronRight, AlertCircle, ExternalLink } from 'lucide-react';

// Force initialize mermaid on load
mermaid.initialize({ startOnLoad: false });

export default function SpecExplorer({ title, description, badge, filePaths, category }) {
    ...
    <div className={`mt-6 ${category === 'non-software' ? 'mb-10' : ''}`}>
      ...
      <div className="bg-bg-primary border border-border-primary rounded-t-xl p-5 border-b-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-text-secondary block mb-1">Final Spec Deliverables</span>
      ...
      <button 
        ...
        style={{ borderTopLeftRadius: 0, borderTopRightRadius: 0, ...(isExpanded ? { borderBottomLeftRadius: 0, borderBottomRightRadius: 0, borderBottom: 'none' } : {}) }}
      ...
      {isExpanded && (
        <div className="border border-border-primary border-t-0 rounded-b-xl overflow-hidden bg-bg-primary flex flex-col">
          ...
          {filePaths.length > 1 && (
            <div className="flex overflow-x-auto bg-[#1e293b] border-b border-[#334155]" role="tablist">
          ...
          <div className="p-6 overflow-y-auto max-h-[600px] bg-bg-primary text-text-primary prose prose-slate max-w-none text-[15px] leading-relaxed relative">
            {isLoading && (
              <div className="flex flex-col items-center justify-center py-20 text-text-secondary gap-3">
                ...
                <span className="font-medium text-sm">Fetching specification from local repository...</span>
              </div>
            )}
            ...
            {!isLoading && error && (
              <div className="bg-danger-bg border border-danger-border p-5 rounded-lg flex flex-col gap-3">
                ...
                <p className="m-0 text-danger-primary/80 text-sm">We encountered an issue fetching the example specification file from the local repository directory (<code>{localSpecPath}</code>).</p>
                <p className="m-0 text-danger-primary/70 text-xs">Technical details: {error}</p>
                <a href={githubFallbackUrl} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1.5 text-danger-primary font-semibold text-sm hover:underline">
                  View the raw file directly on GitHub <ExternalLink size={14} />
                </a>
              </div>
            )}

            {!isLoading && !error && (
              <div dangerouslySetInnerHTML={getHtmlContent()} className="spec-viewer" />
            )}
          </div>
        </div>
      )}
